...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('/api/auth/register', { email, password, name }, { withCredentials: true });
      login();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded shadow-md w-96">
        ...
        <form onSubmit={handleSubmit}>
          <input type="text" placeholder="Full Name" className="w-full p-2 border mb-4 rounded" value={name} onChange={e => setName(e.target.value)} required />
