...
export default function AggregateForm({ initialData = {}, onSubmit, onCancel, isSubmitting }: MaterialFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.Date || !form['Supplier Name'] || !form.Size || !weight || !rate) {
      alert('Please fill all required fields');
      return;
    }
    const amount = weight * rate;
    onSubmit({ ...form, Weight: weight, Rate: rate, Amount: amount });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div><label className="block text-sm font-semibold">Size (mm) *</label><input type="text" name="Size" value={form.Size} onChange={handleChange} required className="w-full border rounded px-3 py-2" /></div>
